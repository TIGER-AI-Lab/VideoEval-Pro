...
class Idefics2Processor(ProcessorMixin):
    ...
    def __call__(
        ...
        if isinstance(images, list) and isinstance(images[0], Image.Image):
            return self._call(text=text, images=images, image_seq_len=image_seq_len, padding=padding, truncation=truncation, max_length=max_length, is_split_into_words=is_split_into_words, add_special_tokens=add_special_tokens, return_tensors=return_tensors)
        elif isinstance(images, list) and isinstance(images[0], list):
            _images = []
            for video in images:
                _images.extend(video)
            image_tokens = "".join(["<image>"] * len(_images))
            text[0] = text[0].replace("<image>", image_tokens)
            return self._call(text=text, images=_images, image_seq_len=image_seq_len, padding=padding, truncation=truncation, max_length=max_length, is_split_into_words=is_split_into_words, add_special_tokens=add_special_tokens, return_tensors=return_tensors)
        else:
