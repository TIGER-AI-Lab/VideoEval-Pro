...
class Qwen2VLProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        if isinstance(images, list) and isinstance(images[0], Image.Image):
            return self._call(images=images, videos=None, text=text, padding=padding, truncation=truncation, max_length=max_length, return_tensors=return_tensors)
        elif isinstance(images, list) and isinstance(images[0], list):
            new_images = []
            for video in images:
                frames = []
                for frame in video:
                    pt_frame = torch.from_numpy(np.array(frame)).to(torch.float32).permute(2, 0, 1)
                    frames.append(pt_frame)
                if len(frames) % 2 != 0:
                    frames = frames[:-1]
                pt_video = torch.stack(frames, dim=0)
                new_images.append(pt_video)
            images = new_images
            return self._call(images=None, videos=images, text=text, padding=padding, truncation=truncation, max_length=max_length, return_tensors=return_tensors)
        else:
