...
class Videochat_flash():
    def __init__(self, model_path:str="OpenGVLab/VideoChat-Flash-Qwen2-7B_res448", device="cuda") -> None:
        """
        Args:
        ...
        # self.model = AutoModel.from_pretrained(model_path, torch_dtype=torch.bfloat16, trust_remote_code=True, attn_implementation=attn_implementation).eval().to(device)
        from models.videochat_flash.modeling_videochat_flash import VideoChatFlashQwenForCausalLM
        self.model = VideoChatFlashQwenForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, attn_implementation=attn_implementation).eval().to(device)
    ...
    def __call__(self, inputs: List[dict], generation_config: dict = None) -> str:
        ...
        generation_config = generation_config if generation_config is not None else dict(
            do_sample=False,
            temperature=0.0,
            max_new_tokens=1024,
            top_p=0.1,
        ...
        response, _ = self.model.chat_new(video_frames=input_images, tokenizer=self.tokenizer, user_prompt=text_prompt, return_history=True, max_num_frames=None, generation_config=generation_config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run VideoChat-Flash inference on VideoEval-Pro dataset')
    ...
    parser.add_argument('--model_path', type=str, default="OpenGVLab/VideoChat-Flash-Qwen2-7B_res448",
                        help='Path to VideoChat-Flash model')
    ...
    model = Videochat_flash(model_path=args.model_path, device=args.device)
