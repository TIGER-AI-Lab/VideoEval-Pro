...
class QWen2_5_VL():
    def __init__(self, model_path="Qwen/Qwen2.5-VL-7B-Instruct", device="cuda") -> None:
        attn_implementation = "flash_attention_2" if is_flash_attn_2_available() else None
        print(f"Using {attn_implementation} for attention implementation")
        self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16, _attn_implementation=attn_implementation).to(device).eval()
        image_processor = Qwen2VLImageProcessor.from_pretrained(model_path)
        tokenizer = AutoTokenizer.from_pretrained(model_path)
        chat_template = tokenizer.chat_template
        self.processor = Qwen2_5_VLProcessor(image_processor=image_processor, tokenizer=tokenizer, chat_template=chat_template)
    ...
    def __call__(self, inputs: List[dict], generation_config: Optional[dict] = None) -> str:
        ...
        if "<video> " in text_prompt:
            text_prompt = text_prompt.replace("<video> ", "<|vision_start|><|video_pad|><|vision_end|>")
        elif "<image> " in text_prompt:
            text_prompt = text_prompt.replace("<image> ", "<|vision_start|><|image_pad|><|vision_end|>")
        conv = self.conv.copy()
        conv.messages = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Qwen2.5-VL inference on VideoEval-Pro dataset')
    ...
    model = QWen2_5_VL(model_path=args.model_path, device=args.device)
