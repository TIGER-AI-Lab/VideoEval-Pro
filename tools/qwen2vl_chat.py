...
class QWen2_VL():
    def __init__(self, model_path="Qwen/Qwen2-VL-7B-Instruct", device="cuda") -> None:
        attn_implementation = "flash_attention_2" if is_flash_attn_2_available() else None
        print(f"Using {attn_implementation} for attention implementation")
        self.model = Qwen2VLForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16, _attn_implementation=attn_implementation).to(device).eval()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Qwen2-VL inference on VideoEval-Pro dataset')
    ...
    model = QWen2_VL(model_path=args.model_path, device=args.device)
