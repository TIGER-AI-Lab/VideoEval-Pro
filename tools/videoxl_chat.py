...
class VideoXL():
    def __init__(self, model_path="sy1998/Video_XL", device="cuda") -> None:

        attn_implementation = "flash_attention_2" if is_flash_attn_2_available() else None
        print(f"Using {attn_implementation} for attention implementation")
        
        tokenizer, model, image_processor, _ = load_pretrained_model(model_path, None, "llava_qwen", device_map=device, attn_implementation=attn_implementation)
        self.model = model.to(device).eval()
        self.model.config.beacon_ratio=[8]
    ...
    def __call__(self, inputs: List[dict], generation_config: Optional[dict] = None) -> str:
        ...
        frame_cnt = len(input_images[0])

        do_resize = False if images.get("metadata", True) else images["metadata"].get("do_resize", False)
        
        input_ids = tokenizer_image_token(prompt, self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).to(self.model.device)
        video_tensor = self.processor.preprocess(np.array(input_images[0]), return_tensors="pt")["pixel_values"].to(self.model.device, dtype=self.model.dtype)
        ...
        num_tokens=TOKEN_PERFRAME * frame_cnt
        beacon_skip_last = beacon_skip_first  + num_tokens

        with torch.inference_mode():
            output_ids = self.model.generate(input_ids, images=[video_tensor],  modalities=["video"],beacon_skip_first=beacon_skip_first,beacon_skip_last=beacon_skip_last, **generation_config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Video-XL inference on VideoEval-Pro dataset')
    ...
    parser.add_argument('--model_path', type=str, default="sy1998/Video_XL",
                        help='Path to Video-XL model')
    ...
    model = VideoXL(model_path=args.model_path, device=args.device)
